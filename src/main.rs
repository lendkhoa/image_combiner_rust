...
fn main() -> Result<(), ImageDataErrors> {
    ...
    output.set_data(combined_data)?; // ? propagate and unwrap the value
...
fn combine_images(image_1: DynamicImage, image_2: DynamicImage) -> Vec<u8> {
    ...
    alternate_pixel(vec_1, vec_2)
}

fn alternate_pixel(vec_1: Vec<u8>, vec_2: Vec<u8>) -> Vec<u8> {
    // If vec_1.len() == 5,
    // vec macro will create a vec<u8> of the same length
...
fn set_rgba(vec: &Vec<u8>, start: usize, end: usize) -> Vec<u8> {
    ...
    for i in start..=end {
        let val: u8 = match vec.get(i) {
            Some(d) => *d, // deference the value
            None => panic!("Index out of bound")
        };
        // we are pushing reference value
        rgba.push(val);
    }
    rgba
}
...
// dimension is a tuple with width and height
